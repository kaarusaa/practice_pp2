...
def _print_contacts(rows):
    ...
    for r in rows:
        ...
        # combine multiple phones into one string
        phone_str = ", ".join(
...
def _fetch_contacts_with_phones(conn, contact_ids):
    """
    Fetch contacts AND their phone numbers from DB.
    - contacts and phones are stored in separate tables
    - this function merges them into one structure
    """
    if not contact_ids:
        return []
...
def filter_by_group():
    ...
    with _conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur: #Create cursor with dictionary output
    ...
    print("\nAvailable groups:") #Show groups to user
    # show groups
    for g in groups:
        print(f"  {g['id']}. {g['name']}")
    choice = input("Enter group number (or name): ").strip() #Get user input

    with _conn() as conn: #Search contacts by group
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # accept id or name
            if choice.isdigit(): #if user entered id
                cur.execute(
                    """
            ...
            ids = [r["id"] for r in cur.fetchall()] #Extract contact IDs

        results = _fetch_contacts_with_phones(conn, ids) #Get full contact info
...
def paginated_browse():
    ...
    with _conn() as conn: #Count total contacts
        with conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM contacts")
            total = cur.fetchone()[0]

    total_pages = max(1, (total + page_size - 1) // page_size) #Calculate total pages
    print(f"\nTotal contacts: {total}  |  Page size: {page_size}") #show initial info
    ...
    while True:
        ...
        with _conn() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                cur.execute(
                    ...
                    (page_size, offset), #Fetch data from DB
                )
                rows = cur.fetchall()

        print(f"\n── Page {page + 1} / {total_pages} ──") #Show page number

        if rows: #Display results
        ...
        cmd = input("[N]ext  [P]rev  [Q]uit: ").strip().lower() #user navigation
        if cmd == "n":
...
def export_to_json(filepath="contacts_export.json"):
    ...
    with _conn() as conn: #Get contact IDs from database
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT id FROM contacts c ORDER BY first_name, last_name")
            ids = [r["id"] for r in cur.fetchall()]
        contacts = _fetch_contacts_with_phones(conn, ids) #Fetch full contact data
