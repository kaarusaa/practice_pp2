...
def draw_hud(): #real-time game information
    ...
    if player and player.nitro_active:
        #Calculate remaining nitro time
...
while running:
        ...
        if event.type == pygame.KEYDOWN:  
            ...
            if state == "SETTINGS":        
                ...
                if event.key == pygame.K_d:
                    ...
                    i = levels.index(settings["difficulty"])
                    settings["difficulty"] = levels[(i + 1) % len(levels)]
    ...
    if state == "MENU":
        ...
        sound_text = f"Sound: {'ON' if settings['sound'] else 'OFF'} (Press 'S' to toggle)"
        difficulty_text = f"Difficulty: {settings['difficulty']} (Press 'D' to cycle)"
        color_text = f"Car Color: {settings['car_color']} (Press 'C' to cycle)"
        screen.blit(font.render(sound_text, True, (255,255,255)), (50, 150))
        screen.blit(font.render(difficulty_text, True, (255,255,255)), (50, 250))
        screen.blit(font.render(color_text, True, (255,255,255)), (50, 350))
        btn_back.draw(screen)
