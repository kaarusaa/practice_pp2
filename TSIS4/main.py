...
def draw_button(text, rect, color, hover_color, mouse_pos):
    ...
    pygame.draw.rect(screen, hover_color if is_hover else color, rect) #Draw button background
    text_surf = font.render(text, True, (255, 255, 255))
    screen.blit(text_surf, (rect.x + (rect.width - text_surf.get_width())//2, rect.y + 10)) #Center text inside button
    return is_hover
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if state == "MENU" and event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE: #Backspace handling
                    username = username[:-1]
                elif event.key != pygame.K_RETURN and len(username) < 15: #Enter key is ignored
